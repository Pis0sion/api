...
class Qiniu
{
     ...
     */
//    public function upload($filepath,$saveName = '', $bucket = '')
//    {
...
//        }
//
//        list($ret, $err) = $uploadManager->putFile($token, $saveName, $filepath);
//        if ($err !== null) {
//            throw new Exception('上传出错' . serialize($err));
//        }
//        return $ret['key'];
//    }
    public function upload($obj,$saveName = '', $bucket = '')
    {
        $token = $this->_getUploadToken($bucket);
        $files = $obj;
        if (empty($files)) {
    ...
    private function _getUploadToken($bucketName)
    {
        ...
        if (!empty($upToken) && empty($bucketName)) {
            ...
            Cache::set('qiniu_upload_token', $upToken,1440);
            return $upToken;
        }
    }
